...
class WindowsWorkAreaReader:
    ...
    def _resolve_api(self) -> SystemParametersInfo:
        ...
        if os.name != "nt":
            raise WorkAreaUnavailableError(
                "Windows work area is unavailable on this platform."
            )
        try:
            return ctypes.windll.user32.SystemParametersInfoW
        except (AttributeError, OSError) as exc:
            raise WorkAreaUnavailableError(
                "Windows work-area API is unavailable."
            ) from exc

    def read(self) -> WorkArea:
        rect = _Rect()
        try:
            succeeded = self._resolve_api()(
                SPI_GETWORKAREA,
                0,
                ctypes.byref(rect),
                0,
            ...
            raise WorkAreaUnavailableError(
                "Windows work area could not be read."
            ) from exc
        if not succeeded:
        ...
        try:
            ...
            raise WorkAreaUnavailableError(
                "Windows returned an invalid work area."
            ) from exc
