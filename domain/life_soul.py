...
class LifeSoulRecord:
    ...
    @classmethod
    def from_dict(cls, payload: Mapping[str, object]) -> LifeSoulRecord:
        if set(payload) != {"character_id", "note"}:
            raise ValueError("Life soul record fields are invalid.")
        character_id = payload["character_id"]
        note = payload["note"]
        if not isinstance(character_id, str) or not isinstance(note, str):
            raise ValueError("Life soul record fields must be strings.")
