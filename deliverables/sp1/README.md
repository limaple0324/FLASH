...
- 版本：SP1 0.1.3 獨立本機正式驗收版（0.1.2 舊工程成品保留）
...
封裝自檢及 `release/sp1` 發布。永久成品為
`FLASH-SP1-Windows-0.1.2-0bd575c-sp1-release.zip`，artifact `8617356997`
與 ZIP SHA-256 均為
`ab02bf2b5d8d8eba3fcf5278329cf4c23741da360b2b824c40ce8473f762629f`。
正式安裝版唯讀模式、交易式更新及一般圖形啟閉均已通過。

## SP1 0.1.3 獨立本機正式驗收成品

2026-07-25～26 工作樹已加入 `B`／`C` 專用安全輸入、三種視窗操作權限、10 份
完整畫面參考、2 個線路數字模板與自動強制登入重連。真實 14 視窗的非前景
背景 `B`／`C` 各通過
14/14；一個真實斷線視窗亦已恢復，最後 14/14 正常、未知 0、失敗 0。
後續多視窗實測已加入最近登入線路 7 的前綴相對辨識，以及匿名指紋保存的
重連階段、公告資格與 60 秒重試時間。三種輸入政策已全部實機通過；另從
14 個同時斷線（含 7 個最小化）完成重連、三種登入後視窗及辨識門檻修正，
最終為 14/14 正常、未知 0、失敗 0、待執行 0。

本次來源已固定為 `28110e6e4857514ee0c99cccacaa2acd5c7b9de7`，278 項
完整測試、Python 編譯、來源自檢與差異檢查通過。獨立成品
`FLASH-SP1-Windows-0.1.3-28110e6-snapshot.zip` 已完成封裝 8/8 自檢、
manifest、ZIP 解壓後驗證及兩次 GUI 啟動／正常關閉，關閉後殘留程序皆為 0。
...
- 永久成品：
  `C:\Users\USER\Documents\輔\SP1成品\FLASH-SP1-Windows-0.1.3-28110e6-snapshot.zip`
- 永久證據：
...
永久保存時使用外層檔名區分，例如
`FLASH-SP1-Windows-<version>-<commit>-snapshot.zip`；內部執行檔仍保留 `FLASH.exe`，
