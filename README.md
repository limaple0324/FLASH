...
來源；目前來源提交 `ead0465` 另包含正常啟動的內建提醒卡工程預覽、安全角色
選擇命令，以及每角色命魂顯示、編輯、保存、清除與失敗隔離。提醒卡選擇、
保存、重啟恢復、清除與浮層失敗隔離也已有自動測試；目前全套 555 項測試、
原始碼編譯與 13 項完整來源自我檢查通過。提醒卡仍是可替換的工程預覽，
不是正式最終視覺；這項來源進度尚未重建 Windows artifact，也不代表玩家
正式選組、魂器、真正遊戲事件或完整 Windows 11 實機驗證已完成。
