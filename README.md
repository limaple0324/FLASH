...
新增工作依 SP1 → SP2 → SP3 依序進行：先完成 SP1 正式驗收，再建立 SP2
獨立新增區域；SP2 完成後建立 SP3 獨立新增區域，並另產生
SP1＋SP2＋SP3 完整整合交付。每個項目開始前先確認需求已討論完成，不自行
補造未定義的欄位、狀態、文案或資料來源。
視窗登記頁目前只確認方向，顯示內容、狀態判定、文案與大量角色版面尚未
確認，因此暫停實作。

## 目前里程碑

- `main`／正式成品：**SP1 0.1.2 工程驗證版**
- `integration/sp2-sp3-sp35`：保留既有 SP2／SP3／SP3.5 累積成果；SP1
  正式完成前不再新增上層功能

整合分支已包含產品與領域資料骨架、工作區與組別級提醒卡、提醒卡生命週期、
歷史紀錄、玩家顯示時間設定、浮層骨架、組別角色清單、角色詳細資料，以及
靈魂石與命魂的正常編輯、安全保存及同名角色安全配對流程。正常 `main()` 也已內建可選的
「基礎提醒卡（預覽）」候選，不依賴測試或外部注入；首次啟動與未選擇時
保持停用。`893f65e` 已通過 485 項自動測試；GitHub Actions run
`30134982561` 已成功建立、下載並獨立重驗
...
來源；`ead0465` 包含安全角色選擇命令與既有命魂手動文字流程。`2d9f878`
已把啟動時真正執行的目標視窗唯讀健康檢查，經由 EventBus 傳入
SP2 執行期唯讀狀態，再由 SP3 首頁安全呈現；控制細節不會傳入首頁，關閉
主視窗後亦會解除訂閱。這條事件鏈不建立活動、不產生提醒卡，也不送出遊戲
輸入。最新來源 `a10c3e2` 另完成「查看目前狀態」時的唯讀重新檢查，以及首頁
提醒卡刷新、角色清單開啟的失敗隔離；提醒卡保留最後有效內容，角色清單安全
清理失敗視窗，玩家只看到固定中文訊息，技術細節只寫入紀錄。目前全套 602 項
測試、原始碼編譯與 14 項完整來源自我檢查通過。
提醒卡仍是可替換的工程預覽，不是正式最終視覺；這項來源進度尚未重建
Windows artifact，也不代表玩家正式選組、真正遊戲活動事件或完整 Windows 11
實機驗證已完成。魂器、寵物天賦、黑曜石、命魂與背包依使用者決定暫停
後續擴充，等資料內容與可信讀取方式討論完成後再接續。
