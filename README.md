...
靈魂石正常編輯與安全保存流程。正常 `main()` 也已內建可選的
...
來源；目前來源提交 `005673f` 再完成正常啟動的內建提醒卡工程預覽接線。
玩家選擇、保存、重啟恢復、清除與浮層失敗隔離均已有自動測試；目前全套
512 項測試、原始碼編譯與完整來源自我檢查通過。這是可替換的工程預覽，
不是正式最終視覺；這項來源進度尚未重建 Windows artifact，也不代表真實
選組、真正遊戲事件或完整 Windows 11 實機驗證已完成。
