...
from ui.life_soul_editor_window import (
    LifeSoulEditorWindow,
    render_life_soul_editor,
)
...
def _editor(**overrides) -> LifeSoulEditorWindow:
    options = {
        "master": object(),
    ...
    return LifeSoulEditorWindow(**options)
...
def test_renderer_shows_initial_note_and_wires_three_actions() -> None:
    ...
    assert labels.widgets[0].options["text"] == "【小古】命魂紀錄"
    assert labels.widgets[1].options["text"] == "可輸入要記住的命魂資訊。"
