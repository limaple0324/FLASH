...
def test_set_for_character_creates_and_persists_normalized_record(tmp_path) -> None:
    path = tmp_path / "soul_stones.json"
    service = SoulStoneService(SoulStoneStore(path))

    record = service.set_for_character(" char-a ", " 本週先保留稀有靈魂石 ")

    assert record == SoulStoneRecord("char-a", "本週先保留稀有靈魂石")
...
def test_set_for_character_replaces_only_target_and_keeps_stable_order(
    ...
    assert service.all() == (updated, SoulStoneRecord("char-b", "第二個角色"))
    assert SoulStoneStore(path).load() == service.all()


def test_clear_for_character_persists_removal_and_missing_is_noop(tmp_path) -> None:
    path = tmp_path / "soul_stones.json"
    store = SoulStoneStore(path)
...
def test_failed_save_keeps_previous_in_memory_record(tmp_path, monkeypatch) -> None:
    path = tmp_path / "soul_stones.json"
    store = SoulStoneStore(path)
