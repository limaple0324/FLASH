...
- 同日伺服器造成 14 個視窗同時斷線（含 7 個最小化）；流程完成斷線、
  ...
  未知 0、失敗 0、待執行 0。
- 尚未完成目前來源的 Windows 成品及最終玩家驗收，因此 SP1 尚未正式完成。
  使用者於 2026-07-26 決定乾淨帳號／另一台電腦驗證延後自行執行，不再
  阻擋本機 SP1 封裝，但不得把該外部驗證寫成已通過。
...
三種輸入模式及動態重連實機驗收已完成；現在執行一次區塊完整測試與編譯，
再建立 Windows 成品並驗收。現有 `C:\Users\USER\Desktop\輔` 專案 junction 與
`C:\Users\USER\Desktop\啟動輔.lnk` 正式程式入口不得互相覆寫。角色名稱
對應仍不猜測。
