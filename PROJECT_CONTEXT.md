...
- `integration/sp2-sp3-sp35` 目前來源功能核對基準：
  `005673fd4b3bcd59ffc55f4573462c24abddce1b`。
...
目前來源提交 `005673f` 已完成正常啟動的內建提醒卡工程預覽接線；全套
512 項測試、全部 Python 原始碼編譯與完整來源自我檢查通過。這項來源進度
...
`005673f` 再完成正常 `main()` 的內建提醒卡工程預覽。後續仍須完成正式最終
視覺、真正遊戲事件與新的 Windows artifact／完整實機驗證；不得補造真實活動、
需求第 3／8 項規則或啟用遊戲輸入。
