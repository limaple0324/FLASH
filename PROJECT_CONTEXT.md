...
- `integration/sp2-sp3-sp35` 目前來源功能核對基準：
  `2d9f878b3f7cd21e2671c0284a178198459154b6`。
...
目前來源提交 `2d9f878` 已完成安全角色選擇命令、既有命魂手動文字流程及
第一條不含遊戲輸入的目標視窗唯讀事件鏈；全套 593 項測試、全部 Python
原始碼編譯與 14 項完整來源自我檢查通過。自我檢查實際結果為
...
`2d9f878` 已包含正常 `main()` 的內建提醒卡工程預覽、安全角色選擇命令、
既有命魂手動文字流程與目標視窗唯讀事件鏈。五項角色資源依使用者最新決定
暫停；下一個小型區塊優先補齊 SP3 唯讀呈現失敗隔離，接著處理玩家可查看的
視窗登記狀態或建立最新 Windows artifact。不得補造真實活動、需求第 3／8 項
規則或啟用遊戲輸入。
