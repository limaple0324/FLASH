...
- `integration/sp2-sp3-sp35` 目前來源功能核對基準：
  `ead0465cf6342afc4cd8af0a9d34fb9e9ef5f890`。
...
目前來源提交 `ead0465` 已完成安全角色選擇命令與每角色命魂正常流程；全套
555 項測試、全部 Python 原始碼編譯與 13 項完整來源自我檢查通過。這項來源
進度尚未建立新的 Windows artifact，也尚未完成完整 Windows 11 實機驗證。
...
`ead0465` 已包含正常 `main()` 的內建提醒卡工程預覽、安全角色選擇命令及
既有命魂手動文字流程。五項角色資源依使用者最新決定暫停；後續先完成
不依賴這五項的可信唯讀事件、玩家可查看狀態、正式最終視覺與新的 Windows
artifact／完整實機驗證；不得補造真實活動、需求第 3／8 項規則或啟用遊戲輸入。
