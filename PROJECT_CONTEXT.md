...
- 本次規則／文件同步分支：`integration/sp2-sp3-sp35`
- 新的 SP1 完成工作基準：`main` 或由 `main` 建立的專用 SP1 分支／工作區
- `integration/sp2-sp3-sp35` 只保留既有 SP2／SP3 累積成果；SP1 正式完成前
  不再新增上層功能
- 2026-07-25 交接基準：`c65b740e9e134057eb04142648ee2635862cb03c`
- 產品決策以正式文件為準；實作狀態以目前程式、測試、提交與自我檢查共同判斷。
- 新增工作嚴格依 SP1 → SP2 → SP3；SP1 正式完成前不新增 SP2，SP2 完成前
  不新增 SP3。既有整合成果保留，不回退。
- 每個項目開始前先確認已討論完成；未定義的欄位、狀態、文案或可信來源暫停。
...
已通過測試的既有命魂程式。後續先處理不依賴這五項、且已討論完成的 SP1
工作；SP1 正式完成後才依序開啟 SP2、SP3。
...
提醒卡與角色清單失敗隔離。五項角色資源依使用者最新決定暫停。「玩家可查看
視窗登記狀態」雖已確認頁面方向，但顯示欄位、狀態分類、判定與文案尚未完成
討論，因此暫停，不自行補造。目前改為先完成已明確的 SP1 驗證、獨立交付與
Windows 成品工作；SP1 正式完成前不新增 SP2／SP3。
...
- 後續依 SP1 → SP2 → SP3 交付；SP1、SP2、SP3 各保留獨立新增區域／交付檔，
  到 SP3 階段另產生 SP1＋SP2＋SP3 完整整合交付。
