...
def _sandbox(workspace: Path, argv: list[str]) -> list[str]:
    ...
    return [bwrap, "--die-with-parent", "--unshare-user", "--unshare-pid", "--unshare-ipc", "--unshare-net", "--unshare-uts", "--unshare-cgroup-try", "--uid", "0", "--gid", "0", "--cap-drop", "ALL", "--new-session", "--ro-bind", "/", "/", "--tmpfs", "/tmp", "--dir", "/tmp/work", "--bind", str(workspace), "/tmp/work", "--proc", "/proc", "--dev", "/dev", "--chdir", "/tmp/work", "--", *argv]
...
def isolation_probe() -> None:
    with tempfile.TemporaryDirectory(prefix="codex-queue-probe-") as temporary:
        ...
        workspace.mkdir()
        script = "test \"$(pwd)\" = \"/tmp/work\"; test -w /tmp/work; test -w /tmp; test ! -e /tmp/work/.git; test -z \"${GITHUB_TOKEN-}\"; test -z \"${OPENAI_API_KEY-}\"; ! touch /etc/codex-queue-probe; grep -q '^CapEff:[[:space:]]*0000000000000000$' /proc/self/status; ! python -c 'import socket; socket.create_connection((\"1.1.1.1\",53),1)'"
