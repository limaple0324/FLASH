...
# Existing-window reattach may use the host's already-resolved exact entry->HWND
# mapping when shortcut/process account identity is unavailable. Normal new-launch
# authorization remains identity-only.
...
if source.count(old) != 1:
    raise SystemExit(f"identityless-reattach rows anchor count={source.count(old)}")
source = source.replace(old, new, 1)
...
if source.count(old) != 1:
    raise SystemExit(f"identityless-reattach condition anchor count={source.count(old)}")
source = source.replace(old, new, 1)
...
# Bind each entry ID to the exact pre-existing HWND already selected by the host.
...
# Fold regressions into the already-invoked ExistingWindowReattachTests class.
